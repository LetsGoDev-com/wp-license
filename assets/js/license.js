(function ($) {
	...
	const LetsgoLicense = {
		...
		bindUIActions: function () {
			...
			// Unassign the license from Letsgodev
			$(document).on('click', '#letsgodev_button_unassign', function(e) {
				e.preventDefault();
				const slug = $(this).data('slug');

				LetsgoLicense.unassignLicense(slug);
			});
		...
		openPopup: function(slug = '') {
			...
			const onSuccess = function (response) {
				...
				$('#letsgodev_box_message').addClass(response.data.class);
				$('#letsgodev_box_message').html(response.data.message);

				if( response.success && response.data.isactive ) {
					const iunassined = '<button id="letsgodev_button_unassign" data-slug="' + slug + '">' + letsgo.unassign_text + '</button>';
					$('#letsgodev_box_button').html(iunassined);
				}
			};
